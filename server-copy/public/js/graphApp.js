...
function sortResponses(responses, sortCoords, newIndex) {
...
graphApp.prototype = {
    ...
    drawQuestion: function(element, choice, userData, answers) {
        ...
        var newIndex = [];
        userData = sortResponses(userData, yCoords, newIndex);
        ...
        //Need to sort answers and values as well
        var swappedValues = new Array(newIndex.length);
        var swappedLabels = new Array(newIndex.length);
        for(i in newIndex) {
            swappedValues[i] = values[newIndex[i]];
            swappedLabels[i] = getLabel(userData[i].value, answers);
        }
